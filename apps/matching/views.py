...
class MatchListView(APIView):
    ...
    def get(self, request):
        ...
        ).values_list('match_id', flat=True).union(
        ...
        matches = Match.objects.filter(
            id__in=match_ids
        ).prefetch_related('legs__sender', 'legs__receiver', 'legs__user_book__book').order_by('-detected_at')
...
class MatchDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        is_participant = match.legs.filter(sender=user).exists() or match.legs.filter(receiver=user).exists()
...
class MatchAcceptView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return Response({'detail': 'Match not found or not active.'}, status=status.HTTP_404_NOT_FOUND)

        # Find the user's leg (as sender)
        try:
            leg = match.legs.get(sender=user)
        except MatchLeg.DoesNotExist:
            return Response({'detail': 'You are not a sender in this match.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        with transaction.atomic():
            ...
            if all(l.status == MatchLeg.Status.ACCEPTED for l in all_legs):
                ...
                # Create the trade
                try:
                    from apps.trading.services.trade_workflow import create_trade_from_match
...
def _notify_ring_cancelled(match: Match, declining_user):
    ...
    for uid in participant_ids:
        try:
            ...
            logger.exception('Failed to notify participant %s of ring cancellation', uid)


def _notify_match_cancelled(match: Match):
    """Notify all participants that a match was cancelled."""
    participant_ids = list(
        match.legs.values_list('sender_id', flat=True).distinct()
    )
    for uid in participant_ids:
        try:
            from apps.notifications.models import Notification
            ...
            logger.exception('Failed to notify participant %s of match cancellation', uid)
