...
class ProposalListCreateView(APIView):
    ...
    def get(self, request):
        ...
        proposals = (
            ...
            .select_related("proposer", "recipient")
            .prefetch_related("items__user_book__book")
            .order_by("-created_at")
        )
...
class ProposalDeclineView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            logger.exception(
                "Failed to notify proposer of declined proposal %s", proposal.pk
            )
...
class TradeListView(APIView):
    ...
    def get(self, request):
        ...
        trade_ids = (
            TradeShipment.objects.filter(sender=user)
            .values_list("trade_id", flat=True)
            .union(
                TradeShipment.objects.filter(receiver=user).values_list(
                    "trade_id", flat=True
                )
            )
        )
...
class TradeDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        is_party = (
            trade.shipments.filter(sender=user).exists()
            or trade.shipments.filter(receiver=user).exists()
        )
        if not is_party:
...
class TradeRateView(APIView):
    ...
    def post(self, request, pk):
        ...
        is_rated_party = (
            trade.shipments.filter(sender=rated_user).exists()
            or trade.shipments.filter(receiver=rated_user).exists()
        )
        if not is_rated_party:
            return Response(
                {"detail": "That user is not part of this trade."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if Rating.objects.filter(trade=trade, rater=request.user).exists():
            return Response(
                {"detail": "You have already rated this trade."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        with transaction.atomic():
            rating = Rating.objects.create(
                ...
                book_condition_accurate=serializer.validated_data[
            ...
            try:
                from apps.ratings.services.rolling_average import (
                ...
                logger.exception(
                    "Failed to recompute rating for user %s", rated_user.pk
                )
            ...
            all_rater_ids = set(
                ...
                str(uid)
                for uid in trade.shipments.values_list("receiver_id", flat=True)
            )
            rated_ids = set(
                str(uid)
