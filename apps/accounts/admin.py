...
class UserAdmin(BaseUserAdmin):
    list_display = [
        'email', 'username', 'account_type', 'is_verified',
        'email_verified', 'is_active', 'total_trades', 'created_at',
    ]
    ...
    fieldsets = (
        ...
        ('Profile', {'fields': ('username', 'account_type', 'is_verified', 'institution_name', 'institution_url')}),
        ('Address (Encrypted)', {'fields': ('full_name', 'address_line_1', 'address_line_2', 'city', 'state', 'zip_code')}),
        ('Email Verification', {'fields': ('email_verified', 'email_verified_at')}),
        ('Stats', {'fields': ('total_trades', 'avg_recent_rating', 'rating_count')}),
        ('Inactivity', {'fields': ('inactivity_warned_1m', 'inactivity_warned_2m', 'books_delisted_at')}),
