...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'account_type', 'institution_name', 'institution_url',
        ]
...
class UserPublicProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'account_type', 'is_verified',
            'institution_name', 'institution_url',
            'total_trades', 'avg_recent_rating', 'rating_count',
            'created_at',
        ]
...
class UserMeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'email', 'username', 'account_type', 'is_verified',
            ...
            'total_trades', 'avg_recent_rating', 'rating_count',
            'max_active_matches',
            'inactivity_warned_1m', 'inactivity_warned_2m', 'books_delisted_at',
            'created_at', 'updated_at', 'last_active_at',
        ]
        read_only_fields = [
            'id', 'email', 'email_verified', 'email_verified_at',
            'is_verified', 'total_trades', 'avg_recent_rating', 'rating_count',
            'max_active_matches', 'inactivity_warned_1m', 'inactivity_warned_2m',
            'books_delisted_at', 'created_at', 'updated_at', 'last_active_at',
        ]
...
class UserMeUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_zip_code(self, value):
        ...
        if value and not re.match(r'^\d{5}(-\d{4})?$', value):
            raise serializers.ValidationError('Enter a valid US ZIP code (e.g. 12345 or 12345-6789).')
        return value
