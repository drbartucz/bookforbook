...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        options = getattr(settings, "DBBACKUP_STORAGE_OPTIONS", {})
        if "s3boto3" in storage_type.lower():
            endpoint = options.get("endpoint_url", "Not set")
            bucket = options.get("bucket_name", "Not set")
            region = options.get("region_name", "Not set")
        ...
        if "s3boto3" in storage_type.lower():
            ...
            try:
                ...
                s3.list_objects_v2(Bucket=options.get("bucket_name"), MaxKeys=1)
