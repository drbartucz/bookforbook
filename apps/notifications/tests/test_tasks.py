...
class TestSendEmailHelper:
    ...
    def test_returns_false_on_exception(self, settings):
        settings.DEFAULT_FROM_EMAIL = 'noreply@bookforbook.com'
        with patch('apps.notifications.email.send_mail', side_effect=Exception('SMTP down')):
...
class TestEmailTemplateFunctions:
    ...
    def test_send_inactivity_warning_2m_email(self, settings):
        ...
        assert result is True
        assert 'removed' in mail.outbox[0].body.lower() or 'delist' in mail.outbox[0].body.lower()
...
from unittest.mock import MagicMock  # noqa: E402 (already imported above, ensure accessible)
...
class TestSendMatchNotificationTask:
    def test_creates_in_app_notifications_for_all_participants(self, settings):
        ...
        MatchLeg.objects.create(match=match, sender=user_a, receiver=user_b, user_book=book_a, position=0)
        MatchLeg.objects.create(match=match, sender=user_b, receiver=user_a, user_book=book_b, position=1)
...
class TestSendBooksDelistedNotificationTask:
    def test_creates_in_app_notification(self, settings):
        ...
        assert Notification.objects.filter(user=user, notification_type='books_delisted').count() == 1
        assert len(mail.outbox) == 1
